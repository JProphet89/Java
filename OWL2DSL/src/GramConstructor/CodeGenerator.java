...
import OWL.Attributes;
import OWL.Ontology;
import OWL.TripleDomain;
...
public class CodeGenerator {
    ...
    private ArrayList<String> subproduction(Node root) {
        ...
        /*for(Iterator k=this.onto.tripledomain.iterator();k.hasNext();){
            TripleDomain doma=(TripleDomain)k.next();
            if(doma.domain.equals(root.concep.name)){
                for(Iterator l=this.onto.triplerange.iterator();l.hasNext();){
                    TripleRange ran=(TripleRange)l.next();
                    if(ran.relation.equals(doma.relation)){
                        triples.add(ran);
                    }
                }
            }
                ...
                if(!visited.contains(range)){
                    ...
                    if(temp.max>0 && temp.min>0){
                        if(temp.max==1 && temp.min==1){
                            ...
                            subproduction+="("+normalize(root.concep.name.toLowerCase())+"_"+relation+"_"+range+"{$"+normalize(root.concep.name.toLowerCase())+".struct."+range+"_"+relation+".add($"+relation+"_"+range+".value);i.objectproperties.add(new Objectproperties($"+relation+"_"+range+".value.replaceAll(\" \",\"_\"),\""+relation_alt+"\"));})";
    ...
    public void generateMainclass(String name,boolean flag,String input,String iri) throws IOException {
