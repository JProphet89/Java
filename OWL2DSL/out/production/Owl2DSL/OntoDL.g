...
@init{
	...
	:
	'Ontology{'
		//Concepts from a ontology and it's atributes
		concepts
...
@init{
	...
	$concept.concep.atribute=new ArrayList<Atributes>();
}
	:
	'{'
...
atribute returns[Atributes atr]
@init{
	$atribute.atr=new Atributes();
}
	:
	'{'
		a1=ID {
...
@init{
	...
	'{'c1=ID r=ID ('min' min=INT)? ('max' max=INT)? c2=ID{
		...
		if($min.text!=null){
			...
			System.out.println("min cardinality="+Integer.parseInt($min.text));
		}else{
			$link.lin.min=0;
		...
		if($max.text!=null){
			...
			System.out.println("max cardinality="+Integer.parseInt($max.text));
		}else{
			$link.lin.max=0;
